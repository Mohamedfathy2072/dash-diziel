...
import TripPhotosSection from "../../pages/TripPhotosSection";
import { useDispatch, useSelector } from "react-redux";
import type { AppDispatch, RootState } from "../../store/store";
import type { Trip } from "../../types/domain";
import useAuth from "../../hooks/useAuth";
import { useEffect } from "react";
import { fetchTripPhotos } from "../../store/tripsSlice";
  ...
  return (
    <>
      <Box className="grid justify-stretch items-start gap-6">
        ...
        <FormSection
          title={t("tripInformation", { defaultValue: "Trip Information" })}
        >
          <Box className="grid justify-stretch items-start grid-cols-2 md:grid-cols-1 gap-5">
            ...
            <Input
              ...
              placeholder={t("scheduledAtPlaceholder", {
                defaultValue: "Select scheduled date and time",
              })}
              optional
            />
          </Box>
        ...
        <FormSection
          title={t("pickupInformation", { defaultValue: "Pickup Information" })}
        >
          <Box className="grid justify-stretch items-start grid-cols-2 md:grid-cols-1 gap-5">
            <Input
              formik={formik}
            ...
            <Input
              ...
              name="pickup_lat"
              type="number"
              placeholder={t("pickupLatitudePlaceholder", {
            ...
            <Input
              ...
              name="pickup_lng"
              type="number"
              placeholder={t("pickupLongitudePlaceholder", {
        ...
        <FormSection
          title={t("destinationInformation", {
            defaultValue: "Destination Information",
          })}
        >
          <Box className="grid justify-stretch items-start grid-cols-2 md:grid-cols-1 gap-5">
            <Input
              ...
              placeholder={t("destinationAddressPlaceholder", {
                defaultValue: "Enter destination address",
              })}
            />
            <Input
              formik={formik}
              label={t("destinationLatitude", {
                defaultValue: "Destination Latitude",
              })}
              name="destination_lat"
              type="number"
              placeholder={t("destinationLatitudePlaceholder", {
                defaultValue: "Enter latitude",
              })}
              optional
            />
            <Input
              formik={formik}
              label={t("destinationLongitude", {
                defaultValue: "Destination Longitude",
              })}
              name="destination_lng"
              type="number"
              placeholder={t("destinationLongitudePlaceholder", {
                defaultValue: "Enter longitude",
              })}
              optional
            />
            <Input
              ...
              type="datetime-local"
              placeholder={t("destinationDatePlaceholder", {
                defaultValue: "Select destination date and time",
              })}
              optional
            />
          </Box>
        ...
        {/* Photos Section */}
        {isSuperAdmin() && 
          (selectedTrip?.status === "accepted" || selectedTrip?.status === "completed") && (
           <FormSection
          title={tTrip("trip_photos", { defaultValue: "Trip Photos" })}
        >
